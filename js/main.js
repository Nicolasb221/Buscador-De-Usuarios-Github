...
const app = Vue.createApp({
  ...
  created() {
    const favoritosGuardados = JSON.parse(
      window.localStorage.getItem("favoritos")
    );
    if (favoritosGuardados?.length) {
      const favoritos = new Map(
        favoritosGuardados.map((favorito) => [favorito.login, favorito])
      );
  ...
  methods: {
    async busqueda() {
      ...
      try {
        ...
        console.log(data);
        this.result = data;
        if (inicioFav === true && !!estaEnFavoritos)
          estaEnFavoritos.ultimaPeticion = Date.now();
      } catch (error) {
        this.error = error;
      } finally {
        this.search = null;
    ...
    añadirFavorito() {
      ...
      console.log(this.favoritos.size);
    ...
    cargarAlmacenamiento() {
      window.localStorage.setItem(
        "favoritos",
        JSON.stringify(this.totalFavoritos)
      );
    },
  },
});
